...
//	ifstream infile;
	...
	start:
//	float Width, Breadth, Side, Radius, Height, Base, Diameter = 0;
	float Area = 0;
/*	
	float Area = 0;
	float Width = 0;
...
-Fixed version name under new convention
...
10-Clean useful commented code

>>> NOTES >>>
1-Area !=0 seems to be ineffective now, and program closes if 0 is entered as dimension (could add an ELSE for that IF or remove the condition completely)
2-Initializing Dimensions as zero in a continuous (separated by commas) list seems to have no effect, but individually initializing DOES
3-When using indiividual initializing, program closes if string is entered as sDim (means it worked?)
*/