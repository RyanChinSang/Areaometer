...
int main()
{
	...
	string Units, ShapeName, ShapeID, helpID, infoID, optionID, restartID, unitsID, diaID, clearID, listID, sDim;
	...
	else 
	{
		...
		if (Mode == "Help")
		{
			if (errorID == "infoID")
			{
				cout << "'" << infoID << "' is an invalid command. Please state either 'Cory' or 'Ryan'." << endl;
				cout << '\n';
				goto backininfoID;
			}// errorID == infoID
			...
			{
				cout << "'" << helpID << "' is an invalid command in [" << Mode << "] mode" << endl;
				cout << '\n';
				goto backinHelp;
		...
		else if (Mode == "Options")
		{
			...
			if (errorID == "diaID")
			{
				cout << "'" << diaID << "' is an invalid command. Please state either 'yes' or 'no'." << endl;
				cout << '\n';
				goto backindiaID;
			}// errorID == diaID
			...
			{
				cout << "'" << clearID << "' is an invalid command. Please state either 'yes' or 'no'." << endl;
				cout << '\n';
				goto backinclearID;
			}// errorID == clearID
			...
			{
				cout << "'" << unitsID << "' is an invalid command. Please state either 'yes' or 'no'." << endl;
				cout << '\n';
				goto backinunitsID;
			}// errorID == unitsID
			...
			{
				cout << "'" << optionID << "' is an invalid command in [" << Mode << "] mode" << endl;
				cout << '\n';
				goto backinOptions;
			}// Invalid input in Options mode [ET]
		...
		else
		{
			...
			system("pause");
			goto restart;
		}// If error occurs outside of Help or Options Mode
		...
		out << "_______________________________________________________________________________ " << endl;
		out << '\n';
		out << '\n';// [MC] error and restart (only when not in other modes)
...
____________________________________________________________________________________________________________________________________
12.03.15 V1.8e
1- Removed IDcls conditional from start of code (logo) and placed that "Back from mode" to memory straight with the "Went to mode" (Fixes [old]TODO#9) not completely
...
9-Add colour
10-Fix all [CNF]
11-Implement means of showing (in memory) error in other mode only, and then output back from that mode
12-*When set to Diameter mode, make Options#2 change to reset to Radius mode.
13-Add "back" functionality for all sub prompts
14-Smoothen the error tolerance when in [Input] mode, just as in [Options] and [Help].
