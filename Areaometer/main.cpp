...
bool checkinput(string Mode, string sDim)
{
	if (Mode == "Input")
	{
		double num;
		istringstream Dim(sDim);
		Dim >> num;
		if (Dim.eof() == false)
		{
			cout << "\"" << sDim << "\" is not a number! Please input a number." << endl;
			return false;
		}
		else
		{
...
void error(string Mode, string errorID)
{
	if (Mode == "Help")
	{
		if (errorID == "infoID")
	...
	else if (Mode == "Options")
	{
		...
		else if (errorID == "MemOpt")
		{
			cout << "ERROR: \"" << MemOpt << "\" is an invalid command. Please state either '1' or '2'." << endl;
			cout << '\n';
		}// errorID == MemOpt

		else if (errorID == "romID")
		{
...
int main()
{
	SetConsoleTitle(TEXT("Areaometer™   [v1.9b]")); // [VC]// Name of the Console Window
	...
	{
		MemClear();
	}// Keep memory clear when opting "not to use memory"
	if (holdUnits == "OFF")
	{
		Units = "units";
	}
	...
	else if (ShapeID == "help" || ShapeID == "Help" || ShapeID == "HELP")
	{
		...
		else if (helpID == "2" || helpID == "AUTHOR" || helpID == "author" || helpID == "AUTHORS" || helpID == "Author" || helpID == "authors" || helpID == "Authors")
		{
			...
			if (infoID == "1" || infoID == "Ryan" || infoID == "ryan" || infoID == "Chin" || infoID == "RYAN" || infoID == "chin" || infoID == "CHIN" || infoID == "sang" || infoID == "Sang" || infoID == "SANG" || infoID == "one" || infoID == "One" || infoID == "ONE")
			{
				cout << '\n';  
		...
		else if (helpID == "3" || helpID == "options" || helpID == "Options" || helpID == "OPTIONS" || helpID == "three" || helpID == "Three" || helpID == "THREE")
		{
			...
			cout << "the way Areaometer can work.\n" << endl;
			system("pause");
			cout << '\n';  
			...
			cout << "   This can be done by typing in \"units\" or \"1\".\n" << endl;
			system("pause");//#1
			cout << '\n';  
			...
			cout << "   This can be done by typing in \"diameter\" or \"2\".\n" << endl;
			system("pause");//#2
			cout << '\n';
			cout << "3- You can change the way Areaometer re-writes to the screen when going \"back\"." << endl;
			cout << "   By default Areaometer reloads past input from RAM - therefore any two" << endl;
			cout << "   consecutive changing of Modes and going \"back\" will erase the past input." << endl;
			cout << "   To avoid this, use reloading from ROM." << endl;
			cout << "   This can be done by typing in \"3\"." << endl;
			cout << "   You can reset this option back to RAM by repeating this process, or" << endl;
			cout << "   You can chose NOT to reload from memory (\"OFF\") by this process.\n" << endl;
			system("pause");//#3
			cout << '\n';  
			cout << "4- You can make Areaometer reset completely." << endl;
			cout << "   This effectivly completely restarts Areaometer." << endl;
			cout << "   This can be done by typing in \"reset\" or \"4\".\n" << endl;
			system("pause");//#4
			cout << '\n';
			cout << "[Help] mode will now restart. ";
			system("pause");
			system("CLS");
			goto help;
	...
	else if (ShapeID == "options" || ShapeID == "option" || ShapeID == "Option" || ShapeID == "OPTIONS" || ShapeID == "OPTION" || ShapeID == "Options")
	{
		...
		if (diamID == "OFF"|| diamID == "ON")
		{ 
			cout << "       ["<<diamID<<"]" << endl;
		}// Status of Option 2
		cout << "[3 - Reload from Memory when going \"back\"]";
		if (ROMmode == "RAM" || ROMmode == "OFF" || ROMmode == "ROM")
		{
			cout << "              ["<<ROMmode<<"]" << endl;
		}// Status of Option 3
		...
		else if (optionID == "3" || optionID == "three" || optionID == "Three" || optionID == "THREE")
		{
			MemOptNum = 0;
			cout << '\n';
			cout << "Select which Memory mode to use: " << endl;
			if (ROMmode == "RAM" || ROMmode == "ROM")
			{
				MemOptNum++;
				cout << "[" << MemOptNum << "- OFF]" << endl;
			}
			if (ROMmode == "OFF" || ROMmode == "ROM")
			{
				MemOptNum++;
				cout << "[" << MemOptNum << "- RAM]" << endl;
			}
			if (ROMmode == "RAM" || ROMmode == "OFF")
			{
				MemOptNum++;
				cout << "[" << MemOptNum << "- ROM]" << endl;
			}
			cout << '\n';// Select which type of memory mode to use

			backinMemOpt:
			// Ask for MemOpt
			cout << "[Input-> Options-> Memory]: ";
			getline(cin, MemOpt);

			if (ROMmode == "OFF")
			{
				if (MemOpt == "1" || MemOpt == "one" || MemOpt == "ONE" || MemOpt == "RAM" || MemOpt == "ram")
				{
					ROMmode = "RAM";
					cout << "Areaometer will now use RAM to reload past input. \n" << endl;
				}// 1- RAM

				else if (MemOpt == "2" || MemOpt == "two" || MemOpt == "TWO" || MemOpt == "ROM" || MemOpt == "rom")
				{
					ROMmode = "ROM";
					cout << "Areaometer will now use ROM to reload past input. \n" << endl;
				}// 2- ROM
			...
			{
				if (MemOpt == "1" || MemOpt == "one" || MemOpt == "ONE" || MemOpt == "OFF" || MemOpt == "off") // OFF
				{
					ROMmode = "OFF";
					cout << "Areaometer will no longer use memory to reload past input." << endl;
					cout << "WARNING: If any, all past input would be lost! \n" << endl;
				}// 1- OFF

				else if (MemOpt == "2" || MemOpt == "two" || MemOpt == "TWO" || MemOpt == "ROM" || MemOpt == "rom") // ROM
				{
					ROMmode = "ROM";
					cout << "Areaometer will now use ROM to reload past input. \n" << endl;
				}// 2- ROM
			...
			else if (ROMmode == "ROM")
			{
				...
				else if (MemOpt == "2" || MemOpt == "two" || MemOpt == "TWO" || MemOpt == "RAM" || MemOpt == "ram") // RAM
				{
					ROMmode = "RAM";
					cout << "Areaometer will now use RAM to reload past input." << endl;
					cout << "WARNING: If any, all past input would be lost! \n" << endl;
				}// 2- RAM
			...
			backinromID:
			// Ask for romID
			cout << "[Input-> Options-> Memory-> Accept?]: ";
			getline(cin, romID);
		
			if (ROMmode == "RAM" || ROMmode == "ROM" || ROMmode == "OFF")
			{
				...
				{
					cout << '\n';
					cout << "Memeory settings cancelled! Restarting [Options] mode . . ." << endl;
					cout << '\n';
					system("pause");
					system("CLS");
					goto options;
			...
			}// Verify if change is what the user wants

		}// optionID == 3 (change Memory mode)
	...
	else if (ShapeID == "exit" || ShapeID == "Exit" || ShapeID == "EXIT" || ShapeID == "Quit" || ShapeID == "QUIT" || ShapeID == "quit")
	{
		goto end;
	}// ShapeID == exit
	...
	{
		MemClear(); // Clears first
		out.close(); // Then exits
	}
	return 0;
...
____________________________________________________________________________________________________________________________________
17.04.15 V1.9a (Refinements, added Memory modes: OFF/RAM/ROM and title to console window)
1-Added <windows.h> for setting the name of the console window
2-Added command to change the name of the console window
3-Added a commented tag VC to find all the lines which need to be changed
4-Port TODOs to 1.9 section
5-Changed all float variables into double to preserve precision and avoid binary memory losses
6-Removed Cory from authors
7-Changed Options 3 mode from saying OFF/ON to RAM/ROM
8-Fixed small runtime spelling error in help mode
9-Made necessary changes to help mode's description of options in options mode
10-Implement a part of Options 3 to now turn off memory, reload from RAM (default) or reload from ROM
11-Fixed small errors in comments
12-Refined how the program displays the status of each option in options mode - linked with the actual variables now
13-Added string variable MemOpt and integer MemOptNum to facilitate turning OFF/RAM/ROM memory options
14-Added error for MemOpt in goto error
15-Refined runtime texts for new implementation
16-Clean up some PUC and sectioned and commented new code
17-Fixed version name

____________________________________________________________________________________________________________________________________
20.04.15 V1.9b (checkinput for sDim, preliminary error function)
1-Added "quit" to exit the program
2-Added checkinput function to check if sDim is a number, forces re-enter if it is not - partially fixes TODO#3
3-Added preliminary error function
...
[EC]  = Error Code
[VC]  = Use Find function to get to loacations of where to change the version name, for each new version
...
3-Add improved error tolerance (eg. report error and force re-enter if sDim is a string)
4-Make some sort of realistic units library for #3
5-Force the user to enter only "m" or "cm" (report error and force re-enter if Units does not obey standard set of Units library)
6-*Use SystemCLS to make interface much cleaner and program-like
7-Clean all unuseful commented code [PUC] and [FUC]
8-Fix all [CNF]
9-Add "back" functionality for all sub prompts
10-Add "history" funtion - so not loading to ROM, but keeps track of everything in [Input]
11-Add error function - instead of goto error; error(string errorID)
12-Add functionality where the program tells you when ROM or RAM (arrays) would be getting full/is full.

[AIM] Version 2.0 to have memory, colour, (error)sound and much functionality!
[V1.8] - Memory implementation phase and added functionality
...
2-A maximum of only 300 lines can be shown at any one instance in console
*/